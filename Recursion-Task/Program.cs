...
{ 
    ...
    // IMPORTANT NOTE
    // Given any number N, N%10 will give you the rightmost digit and N/10 will remove the right most digit
    internal class Program
    {
        static int[] arrrs = new int[] { 1, 2, 3, 4, 5, 6, 7, 8, 9 };
        static void Main(string[] args)
        {
           
            Console.WriteLine(GetFibaCursion(7));
          //Console.WriteLine(isss);
            Console.Read();
        }


        // Get Power recursively, A^B
        ...
        //Write a program in to print even or odd numbers in a given range using recursion
        ...
        public int countHi2(String str)
        {
            ...
            if (sub.StartsWith("hi"))
            {
                return 1 + countHi2(str.Substring(2));
            }

            if (sub.StartsWith("xhi"))
            {
                return countHi2(sub.Substring(3));
            }

            if (!sub.StartsWith("x") && !sub.Contains("hi"))
            {
